...
class D3D12AppDefault
{
	...
	Microsoft::WRL::ComPtr<ID3D12Fence>					m_Fences[FRAME_COUNT];
	...
	FPSCounter											m_FPSCounter{"d3d12_app_default_fps_counter.log"};
	...
	bool InitRTVsFromSwapChain()
	{
		...
		hr = m_Device->CreateDescriptorHeap(&rtvHeapDescriptor, IID_PPV_ARGS(&m_RTVDescriptorHeap));
	...
	{
		// CUBE:
		m_ModelPosition.x += 0.002f;

		if(m_ModelPosition.x > 1.0f)
		{
			m_ModelPosition.x = -1.0f;
		}
	...
	void Setup()
	{
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_RTVDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), m_CurrentFrameIndex, m_RenderTargetViewDescriptorSize);
	...
	void LoadMeshes()
	{
		...
		mesh01.LoadMesh(*m_Device.Get(), *m_CommandList.Get(), &data, &indices);
...
public:
	...
	void Render()
	{
		...
		mesh01.Render(*m_CommandList.Get());
