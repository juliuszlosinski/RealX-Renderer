...
private:
    ...
    int LoadImageDataFromFIle(BYTE** imageData, D3D12_RESOURCE_DESC& resourceDescription, LPCWSTR fileName, int& bytesPerRow)
    {
        ...
        m_ImageSize = bytesPerRow * textureHeight;

        *imageData = (BYTE*)malloc(m_ImageSize);

        if (imageConverted)
        {
            hr = pWicConverter->CopyPixels(0, bytesPerRow, m_ImageSize, *imageData);
            if (FAILED(hr))
            {
                return 0;
            }
        ...
        {
            hr = pWicFrame->CopyPixels(0, bytesPerRow, m_ImageSize, *imageData);
            if (FAILED(hr))
            {
                return 0;
            }
        ...
        resourceDescription.Alignment = 0; // May be 0, 4 KB, 64 KB or 4 MB. 0 will let runtime decide between 64 KB and 4 MB (4 MB for multi-sampled textures).
...
public:
    ...
    {
        m_ImageSize = LoadImageDataFromFIle(&m_pImageData, m_TextureDescriptor, pathToImage, m_ImageBytesPerRow);
